...
|---|---|---|
| `mesh_size` | `0.06` | horizontal target edge length |
| `nz` | `12` | number of extrusion layers |
...
The cell and node counts depend on the mesh generated by Gmsh.

Both drivers execute their default solve when run or included. For a smaller
headless run, include the shared definitions from the repository root:

```julia
include("examples/miniapps/stokes/sinking_block/sinking_block_3D_setup.jl")
forward = run_sinking_block_3d(;
    mesh_size = 0.25, nz = 4, half_width = 0.25,
    write_output = false, verbose = false,
)
adjoint = solve_sinking_block_adjoint_3d(forward)
```

Use `--project=examples` for this workflow. The shared file defines the forward
and adjoint functions without launching a solve.
