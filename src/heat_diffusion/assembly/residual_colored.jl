...
Elements within a group share no nodes, so the scatter into `R`, `∂R∂T` and
`PC` needs no atomics. Set `compute_jacobian=true` to also fill the absolute
