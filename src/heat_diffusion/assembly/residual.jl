...
function assemble_diffusion_matrices_colored!(R, ∂R∂T, PC, T, T0, el2n, geo, el_groups,
                                               element::ReferenceElement{Te}, phases, k, Cp, ρ0, α, K, P, Δt, source, do_∂R∂T, backend, workgroup) where Te<:AbstractElement{2, N} where N
    Nq = shape_function_values(element)

    fill!(R, 0)
    for group in el_groups
        residual_colored_kernel!(backend, workgroup)(
            R, T, T0, source, el2n, geo, phases, k, Cp, ρ0, α, K, P, Δt, Nq, group, Val(N);
    ...
    if do_∂R∂T
        ...
        for group in el_groups
            jacobian_colored_kernel!(backend, workgroup)(
                ∂R∂T, PC, T, T0, source, el2n, geo, phases, k, Cp, ρ0, α, K, P, Δt, Nq, group, Val(N);
                ndrange = length(group),
            )
        end
    end
...
@kernel function residual_colored_kernel!(R, @Const(T), @Const(T0), @Const(source), @Const(el2n), @Const(geo), @Const(phases), k, Cp, ρ0, α, K, @Const(P), Δt, Nq, @Const(group), ::Val{N}) where N
    i = @index(Global)
    iel = group[i]
    local_nodes, Re = element_residual(T, T0, source, el2n, geo, phases, k, Cp, ρ0, α, K, P, Δt, Nq, iel, Val(N))
...
@kernel function jacobian_colored_kernel!(∂R∂T, PC, @Const(T), @Const(T0), @Const(source), @Const(el2n), @Const(geo), @Const(phases), k, Cp, ρ0, α, K, @Const(P), Δt, Nq, @Const(group), ::Val{N}) where N
    i = @index(Global)
    iel = group[i]
    local_nodes, rowsums, diags = element_jacobian(T, T0, source, el2n, geo, phases, k, Cp, ρ0, α, K, P, Δt, Nq, iel, Val(N))
    for (j, inod) in enumerate(local_nodes)
