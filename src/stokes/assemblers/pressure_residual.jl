...
step. `Qloc` is the nodal volumetric source/sink rate [s⁻¹], with positive
values denoting expansion and negative values contraction. `Nq` contains
pressure shape-function values at pressure quadrature points.

Weak form per pressure node `i`:

    RPᵢ = ∫ Nᵢ (−∇·v − ∂P/∂t/ηb + α ∂T/∂t + Q) dΩ
...
# Compatibility entry point: an omitted source is identically zero.
@inline integrate_PH_pressure_residual(v, P_loc, P0loc, Tloc, T0loc,
    geo_v_el, geo_P_el, phase_loc, α, ηb, Δt, Nq) =
    integrate_PH_pressure_residual(v, P_loc, P0loc, Tloc, T0loc, zero(P_loc),
        geo_v_el, geo_P_el, phase_loc, α, ηb, Δt, Nq)
...
and `Q` are pressure-node fields. `Q` is the volumetric source/sink rate [s⁻¹];
positive values denote expansion and negative values contraction.
...
bulk viscosity `NTuple`s. The compatibility method without `Q` uses a zero
source field.
"""
...
function assemble_pressure_residual_matrices_atomix!(
    ...
    return assemble_pressure_residual_matrices_atomix!(
        RP, vx, vy, P, P0, T, T0, zero(P), el2n_v, el2nP, geo_v, geo_P, nels,
        element_v, element_P, phases, α, ηb, Δt, backend, workgroup,
    )
end
...
function assemble_pressure_residual_kernel!(
    RP, vx, vy, P, P0, T, T0, el2n_v, el2nP, geo_v, geo_P, nels,
    phases, α, ηb, Δt, NqP, nv, np, workgroup,
)
    return assemble_pressure_residual_kernel!(
        RP, vx, vy, P, P0, T, T0, zero(P), el2n_v, el2nP, geo_v, geo_P, nels,
        phases, α, ηb, Δt, NqP, nv, np, workgroup,
...
@kernel function pressure_residual_atomic_kernel!(
    ...
    iel = @index(Global)
    local_nodes_P, Re = pressure_element_residual(vx, vy, P, P0, T, T0, Q, el2n_v, el2nP, geo_v, geo_P, phases, α, ηb, Δt, NqP, iel, Val(NV), Val(NP))
    # P is discontinuous here, so element pressure DoFs are not shared.
    _add_local!(RP, local_nodes_P, Re, Val(false))
end

"""
    pressure_element_residual(vx, vy, P, P0, T, T0, Q, el2n_v, el2nP,
                               geo_v, geo_P, phases, α, ηb, Δt, NqP, iel, Val(NV), Val(NP))

Gather element-local nodal values, including the pressure-node volumetric
source/sink field `Q`, and integrate the Stokes pressure residual for element
`iel`. The compatibility method without `Q` uses a zero source field.

Returns `(local_nodes_P, Re)` ready for global scatter into `RP`.
"""
@inline function pressure_element_residual(vx, vy, P, P0, T, T0, Q, el2n_v, el2nP,
    ...
    Qloc      = _gather_local(Q,  local_nodes_P, Val(NP))
...
@inline pressure_element_residual(vx, vy, P, P0, T, T0, el2n_v, el2nP,
