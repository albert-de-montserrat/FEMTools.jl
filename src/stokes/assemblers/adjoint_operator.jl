...
- `B` is the total `∂Rv/∂P`, `2NV`×`NP` per element, likewise differentiated with
  `Pnum` formed inline, so it carries the augmentation term `Bnum·(γP/M_P)·D`.
- `C` is `∂RP/∂v`, `NP`×`2NV` per element.
- `D` is `∂RP/∂P`, `NP`×`NP` per element. It vanishes only in the incompressible
  limit: `RP` stores pressure elastically through `-(P-P0)/(ηb·Δt)`, so dropping
  it would leave the adjoint wrong by `O(1/(ηb·Δt))` at finite bulk viscosity.

Storage is `≈(4NV² + 4NV·NP + NP²)` floating-point numbers per element — 289 for
T7/P1-disc in two dimensions, about 2.3 kB.
"""
struct FrozenAdjointOperator{TA, TB, TC, TD}
    A::TA
    B::TB
    C::TC
    D::TD
...
"""
    element_adjoint_operator_blocks(...) -> (local_nodes_v, local_nodes_P, A, B, C, D)

Build the four transposed-operator blocks for element `iel` at the current
...
@inline function element_adjoint_operator_blocks(
    ...
    # Total ∂Rv/∂P: Pnum is formed inline from the element pressures, exactly as
    # in the forward residual, so B carries the augmentation through Pnum(P) just
    # as A carries it through Pnum(v).
    ...
    return local_nodes_v, local_nodes_P, A, B, C, D
end

@kernel function adjoint_operator_assembly_kernel!(
        Ablocks, Bblocks, Cblocks, Dblocks,
    ...
    iel = @index(Global)
    local_nodes_v, _, A, B, C, D = element_adjoint_operator_blocks(
        vx, vy, P, P0, T, T0, el2n_v, el2nP, geo_v, geo_P,
        phases_v, phases_P, τ_old, plastic, η, G, α, ρ0, K, g, Tref, ηb, Δt, γ_eff,
        MP, Nq, NqP, iel, Val(NV), Val(NP),
    )
    ...
    Cblocks[iel] = C
    Dblocks[iel] = D
    # The solve applies Aᵀ, so a Gershgorin bound on it needs the row sums of the
    # transpose, which are the column sums of A.
    rowsums_x = SVector{NV}(ntuple(j -> sum(abs(A[i, j]) for i in 1:2NV), Val(NV)))
    rowsums_y = SVector{NV}(ntuple(j -> sum(abs(A[i, NV + j]) for i in 1:2NV), Val(NV)))
    for (i, inod) in enumerate(local_nodes_v)
...
function assemble_adjoint_operator(
    ...
    Dblocks = similar(dr.vx, SMatrix{NP, NP, Tv, NP * NP}, nels)
    ...
    KA.synchronize(backend)
    return FrozenAdjointOperator(Ablocks, Bblocks, Cblocks, Dblocks)
end

@kernel function adjoint_operator_apply_kernel!(
        dvx, dvy, dP,
        @Const(Ablocks), @Const(Bblocks), @Const(Cblocks), @Const(Dblocks),
    ...
    resp = transpose(Bblocks[iel]) * λv + transpose(Dblocks[iel]) * λP_loc
...
function apply_adjoint_operator!(
    ...
    adjoint_operator_apply_kernel!(backend, workgroup)(
        dvx, dvy, dP, op.A, op.B, op.C, op.D, λvx, λvy, λP,
