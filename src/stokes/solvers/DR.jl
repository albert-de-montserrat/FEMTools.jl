...
filled before calling. The solver assembles `dr.M_V`, the positive velocity mass
diagonal, from `geo_v` before evaluating convergence. Both momentum and pressure
convergence errors use mass-weighted RMS norms, so locally refined regions are
weighted by physical support rather than node count. The two forms differ only
in how the precomputed element geometry is supplied: as a `MixedMeshCache` or as
    ...
    for itPH in 1:max_ph_iterations
        ...
        while err_v_inner > target_v && itPT < iterMax && iter < total_iterMax
            ...
            if iszero(itPT % nout) || itPT == iterMax || iter == total_iterMax
                ...
                # λmin is a finite difference of the residual across the step just
                # taken, so Rv_x0 and Rv_x must bracket exactly that step.
