function update_poisson_R!(R, T, b, κ, grid, element)
    nel = size(grid)
    ...
    for iblock in 1:num_blocks
        ...
        for iel in block_start:block_end
            iel > nel && break
            local_update_poisson_R!(R, T, b, κ, grid, element, iel)
        end
    end
...
function local_update_poisson_R!(R, T, b, κ, grid, element, iel)
    # local residual vector
    RL        = local_SVector(element)
    # pre-fetch local vectors
    el_coords = getelementcoords(grid, iel)
    T_element = getelementfield(T, grid, iel)
    b_element = getelementfield(b, grid, iel)
    κ_element = getelementfield(κ, grid, iel)
    ...
    for ip in eachindex(element.ip) 
        ...
        K    = (∇N_global' * ∇N_global) .* κ_element .* detJ_ω
        # local mass matrix
        rhs  = @. b_element * N * detJ_ω
        # assemble residual
        RL += K * T_element - rhs
    end
    ...
    assemble_sparse_vector!(R, RL, global_dofs)
end
