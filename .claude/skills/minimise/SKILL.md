...
4. **Run the focused tests.** Test files live in `test/test_*.jl` and are all
   included by `test/runtests.jl`. A single file can be run with the shared
   preamble from `runtests.jl`:

   ```bash
   julia --project -e '
   using Test, DomainSets, FEMTools, SparseArrays, StaticArrays
   using DomainSets: ×
   const FP64 = Float64; const FP32 = Float32
   include("test/test_mesh.jl")'
   ```

   Run the full suite (`julia --project -e "using Pkg; Pkg.test()"`) before the
   final report. Confirm all tests pass before continuing.
...
- Changes to `examples/` are demos, not library code: keep them out of a
  src/ bug-fix diff unless the example is the regression test.
