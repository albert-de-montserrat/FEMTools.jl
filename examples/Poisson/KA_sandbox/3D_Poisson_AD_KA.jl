...
function main(nels, element)
    ...
    ΓD = TDev(in.(Array(mesh.coords), boundary(Ω)))
    ...
    assemble_diffusion_matrices_atomix!(R, ∂R∂H, PC, H_FEM, mesh.el2n, geo, mesh.nels, element, D, source, true)
    ...
    ncheck = 100
    # for it = 1:2000
    @timeit to "solver" for it = 1:1_000
        if mod(it, ncheck) == 0
            copyto!(R0, R)
        end
