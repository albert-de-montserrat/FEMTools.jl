...
function main(; nsteps = 15, mesh_cells = (32, 32) .* 2, Δt = 1/6, show_plot = true)
    ...
    for istep in 1:nsteps
        ...
        λmax_vx = maximum(dr.∂Rv∂v.x ./ dr.PC_v.x)
        λmax_vy = maximum(dr.∂Rv∂v.y ./ dr.PC_v.y)
        ...
        for itPH in 1:1000
            ...
            FEMTools.apply_dirichlet!(dr.Rv.x, vx_nodes, bc_vx.zero_vals, backend, workgroup)
            FEMTools.apply_dirichlet!(dr.Rv.y, vy_nodes, bc_vy.zero_vals, backend, workgroup)
            ...
            err_v = max(norm(dr.Rv.x), norm(dr.Rv.y)) / (2 * √mesh_stokes.nnodes)
            if itPH == 1
            ...
            while err > ϵ_vel && itPT ≤ iterMax
                ...
                FEMTools.apply_dirichlet!(dr.Rv.x, vx_nodes, bc_vx.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.x, vx_nodes, bc_vx.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.Rv.y, vy_nodes, bc_vy.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.y, vy_nodes, bc_vy.zero_vals, backend, workgroup)
                ...
                FEMTools.apply_dirichlet!(dr.v.x, vx_nodes, bc_vx_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.v.y, vy_nodes, bc_vy_vals, backend, workgroup)

                # Inner convergence check + damped step-size update
                if iszero(iter % nout)
                    err_v_inner = max(norm(dr.Rv.x), norm(dr.Rv.y)) / (2 * √mesh_stokes.nnodes)
                    if iter == nout
                    ...
                    λmin_vx = _λmin(α_vx, dr.∂v∂τ.x, dr.Rv.x .- dr.Rv0.x, dr.PC_v.x)
                    λmin_vy = _λmin(α_vy, dr.∂v∂τ.y, dr.Rv.y .- dr.Rv0.y, dr.PC_v.y)
