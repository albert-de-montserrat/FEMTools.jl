...
function main(;
    ...
    fill!(dr.PC_v.x, 1 / Δτ_V_seed)
    fill!(dr.PC_v.y, 1 / Δτ_V_seed)
    ...
    while t < t_end - eps(t_end)
        ...
        λmax_vx0 = max(maximum(dr.∂Rv∂v.x ./ dr.PC_v.x), eps(Float64))
        λmax_vy0 = max(maximum(dr.∂Rv∂v.y ./ dr.PC_v.y), eps(Float64))
        ...
        for itPH in 1:niter_PH
            ...
            FEMTools.apply_dirichlet!(dr.Rv.x, vx_nodes, vx_bc, backend, workgroup)
            FEMTools.apply_dirichlet!(dr.Rv.y, vy_nodes, vy_bc, backend, workgroup)
            ...
            err_v = (norm(dr.Rv.x) + norm(dr.Rv.y)) / (2 * sqrt(mesh_stokes.nnodes) * Rv_ref)
            if itPH == 1
            ...
            while err > ϵ_vel && itPT ≤ niter_inner
                ...
                FEMTools.apply_dirichlet!(dr.Rv.x, vx_nodes, vx_bc, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.x, vx_nodes, vx_bc, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.Rv.y, vy_nodes, vy_bc, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.y, vy_nodes, vy_bc, backend, workgroup)

                FEMTools.stokes_update_rate!(dr.∂v∂τ.x, dr.Rv.x, dr.PC_v.x, β_vx, mesh_stokes.nnodes, backend, workgroup)
                FEMTools.stokes_update_variable!(dr.v.x, dr.∂v∂τ.x, -α_vx, mesh_stokes.nnodes, backend, workgroup)
                FEMTools.stokes_update_rate!(dr.∂v∂τ.y, dr.Rv.y, dr.PC_v.y, β_vy, mesh_stokes.nnodes, backend, workgroup)
                FEMTools.stokes_update_variable!(dr.v.y, dr.∂v∂τ.y, -α_vy, mesh_stokes.nnodes, backend, workgroup)

                FEMTools.apply_dirichlet!(dr.v.x, vx_nodes, vx_bc, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.v.y, vy_nodes, vy_bc, backend, workgroup)

                if do_jac
                    err_v_inner = (norm(dr.Rv.x) + norm(dr.Rv.y)) / (2 * sqrt(mesh_stokes.nnodes) * Rv_ref)
                    if iter == 1
                    ...
                    λmax_vx = max(maximum(dr.∂Rv∂v.x ./ dr.PC_v.x), eps(Float64))
                    λmax_vy = max(maximum(dr.∂Rv∂v.y ./ dr.PC_v.y), eps(Float64))
                    Δτ_vx = 2 / sqrt(λmax_vx) * dr.CFL_v
                    Δτ_vy = 2 / sqrt(λmax_vy) * dr.CFL_v

                    λmin_vx = itPT == 1 ? 0.0 : _λmin(α_vx, dr.∂v∂τ.x, dr.Rv.x .- dr.Rv0.x, dr.PC_v.x)
                    λmin_vy = itPT == 1 ? 0.0 : _λmin(α_vy, dr.∂v∂τ.y, dr.Rv.y .- dr.Rv0.y, dr.PC_v.y)
    ...
    # the stress panel. Re-fetched here because dr.v.x/vy hold the final state.
    vx_cpu = Array(dr.v.x)
    vy_cpu = Array(dr.v.y)
