...
function main(;
    ...
    while t < t_end - eps(t_end)
        ...
        for itPH in 1:niter_PH
            ...
            FEMTools.apply_dirichlet!(dr.Rv.x, lr_nodes, bc_vx.zero_vals, backend, workgroup)
            FEMTools.apply_dirichlet!(dr.Rv.y, tb_nodes, bc_vy.zero_vals, backend, workgroup)
            ...
            while err > ϵ_vel && itPT ≤ niter_inner
                ...
                FEMTools.apply_dirichlet!(dr.Rv.x, lr_nodes, bc_vx.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.x, lr_nodes, bc_vx.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.Rv.y, tb_nodes, bc_vy.zero_vals, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.∂v∂τ.y, tb_nodes, bc_vy.zero_vals, backend, workgroup)

                update_rate!(dr.∂v∂τ.x, dr.Rv.x, dr.PC_v.x, β_vx, mesh_stokes.nnodes)
                update_variable!(dr.v.x, dr.∂v∂τ.x, -α_vx, mesh_stokes.nnodes)
                update_rate!(dr.∂v∂τ.y, dr.Rv.y, dr.PC_v.y, β_vy, mesh_stokes.nnodes)
                update_variable!(dr.v.y, dr.∂v∂τ.y, -α_vy, mesh_stokes.nnodes)

                FEMTools.apply_dirichlet!(dr.v.x, lr_nodes, bc_vx_lr, backend, workgroup)
                FEMTools.apply_dirichlet!(dr.v.y, tb_nodes, bc_vy_tb, backend, workgroup)
