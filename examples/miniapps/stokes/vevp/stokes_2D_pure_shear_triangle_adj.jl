...
observation box contribute nothing. `FEMTools.element_geometry(geo, iel)[q][2]` is the physical quadrature
...
@kernel function objective_vy_kernel!(
    ...
    for q in eachindex(Nq)
        ...
        if xmin < xq[1] < xmax && ymin < xq[2] < ymax
            dΩ = FEMTools.element_geometry(geo, iel)[q][2]
...
@kernel function momentum_moduli_contraction_kernel!(
    ...
    Re_x, Re_y = FEMTools.integrate_momentum_residual(
        (vx_e, vy_e), P_e, nothing, T_e, FEMTools.element_geometry(geo, iel), phase_e,
...
@kernel function pressure_moduli_contraction_kernel!(
    ...
    RP_e = FEMTools.integrate_PH_pressure_residual(
        ...
        FEMTools.element_geometry(geo_v, iel), FEMTools.element_geometry(geo_P, iel), phase_e,
...
function moduli_sensitivities(
    ...
    Enzyme.autodiff_deferred(
        ...
        Enzyme.Const(dr.v.x), Enzyme.Const(dr.v.y), Enzyme.Const(dr.P), Enzyme.Const(dr.T),
    ...
    Enzyme.autodiff_deferred(
        ...
        Enzyme.Const(dr.v.x), Enzyme.Const(dr.v.y),
...
function main(;
    ...
    objective_vx = zero(dr.Rv.x)
    objective_vy = zero(dr.Rv.y)
    ...
    objective = dot(objective_vy, dr.v.y)
    ...
    function plot_summary()
        ...
        vx_cpu, vy_cpu, P_cpu = Array(dr.v.x), Array(dr.v.y), Array(dr.P)
