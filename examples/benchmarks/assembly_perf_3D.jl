...
function color_element_batches(nels::NTuple{3, <:Integer})
    nx, ny, nz = nels
    batches = [Int[] for _ in 1:8]

    iel = 1
    for ez in 0:(nz - 1), ey in 0:(ny - 1), ex in 0:(nx - 1)
        color = 1 + (ex & 1) + 2 * (ey & 1) + 4 * (ez & 1)
...
function assemble_diffusion_matrices!(
    ...
    coords = element_coordinate_matrix(mesh, local_nodes, Val(3))
...
function quadrature_cache(element::ReferenceElement{3})
...
function assemble_diffusion_matrices_color_coded!(K, M, F, mesh, element::ReferenceElement{3, N}, κ, source, colors) where {N}
...
function assemble_diffusion_matrices_nocolor_coded!(K, M, F, mesh, element::ReferenceElement{3, N}, κ, source) where {N}
    ip, Nq, ∂N∂ξq = quadrature_cache(element)
...
function assemble_diffusion_matrices_atomics!(K, M, F, mesh, element::ReferenceElement{3, N}, κ, source) where {N}
    ip, Nq, ∂N∂ξq = quadrature_cache(element)
...
function foo()
    ...
    axislegend(ax)
    ax.xlabel = "# linear 3D hexahedral elements"
    ...
    ax_speedup.xlabel = "# linear 3D hexahedral elements"
    ax_speedup.ylabel = "speedup"
    rowsize!(f.layout, 1, Relative(2 / 3))
    rowgap!(f.layout, 20)
    save("perf_assembly_3D_$(nt)threads.png", f)
    f
end
