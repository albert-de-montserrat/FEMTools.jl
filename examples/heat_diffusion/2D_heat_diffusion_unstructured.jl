...
function main(; max_area=1e5)
    ...
    α   = (FP(3e-5),)
    K   = (FP(1e11),)
    g   = FP(9.81)                          # gravitational acceleration [m s⁻²]
    ...
    # Lithostatic pressure: P = ρ0 * g * depth,  depth = -y  (y ≤ 0)
    P_litho = FP[ρ0[1] * g * (-coords_cpu[i][2]) for i in eachindex(coords_cpu)]
    copyto!(dr.P, P_litho)
    ...
    T_obs  = Observable(Array(dr.T))   # driven by the time loop
    t_obs  = Observable(0.0)

    fig = Figure(size = (720, 640))
    ax  = Axis(fig[1, 1];
               title   = @lift("Heat diffusion  t = $(round($t_obs / (1e3*365.25*24*3600); digits=1)) kyr"),
               aspect  = DataAspect(),
    ...
    for (cx, cy, r) in holes
        lines!(ax, cx .+ r .* cos.(θ_c), cy .+ r .* sin.(θ_c); color = :white, linewidth = 2)
    end
