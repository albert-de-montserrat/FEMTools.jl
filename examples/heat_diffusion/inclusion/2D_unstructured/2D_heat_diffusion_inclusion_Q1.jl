...
function build_mesh_Q1(; L, mesh_size)
    gmsh.initialize()
    try
        gmsh.option.setNumber("General.Terminal", 0)
        gmsh.option.setNumber("Mesh.RecombineAll", 1)
        gmsh.option.setNumber("Mesh.RecombinationAlgorithm", 1)
        gmsh.model.add("heat_inclusion_q1")
        ...
        quad_idx = findfirst(==(3), elemtypes)
        isnothing(quad_idx) && error("No 4-node quadrilateral elements found in Gmsh mesh.")
        quad_flat = elemnodetags[quad_idx]
        nels = length(quad_flat) ÷ 4
        el2n = Matrix{Int32}(undef, 4, nels)
        for iel in 1:nels, i in 1:4
            el2n[i, iel] = tag2idx[Int(quad_flat[4(iel - 1) + i])]
        end
...
function main(nels; do_plot = true, nsteps = 50, verbose = true)
    ...
    coords_cpu, el2n_cpu, Γ = build_mesh_Q1(; L, mesh_size = h)
