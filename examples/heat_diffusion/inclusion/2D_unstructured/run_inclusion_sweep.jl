# Runs the unstructured inclusion variants (Q1, Q2, T1, T2) over a set of
# target edge resolutions and compares number of elements, number of DoFs, PT
# iteration count, and solver wall time.
#
# T variants use Triangulate.jl. Q variants use Gmsh.jl recombined quadrilateral
# meshes. `RUN_SWEEP` is defined in each module beforehand so scripts skip their
...
const sizes  = [(16, 16), (32, 32), (64, 64), (128, 128), (256, 256)]
const nsteps = 10
...
function load_variant(name, file)
    mod = Module(Symbol(:SweepUnstructured, name))
...
function run_sweep()
    ...
    for (name, file) in variants
        ...
        results[name] = map(sizes) do sz
            ...
            @printf("  target %-9s nels = %7d  ndofs = %8d  PT iters = %6d  time = %9.3f s\n",
...
function summarize(results)
    ...
    @printf("%-4s %-10s %9s %9s %10s %12s\n", "el", "target", "nels", "ndofs", "PT iters", "time [s]")
    for (name, _) in variants
        for (sz, r) in zip(sizes, results[name])
            run_sz = sweep_size(name, sz)
            @printf("%-4s %-10s %9d %9d %10d %12.3f\n",
    ...
    # PT iterations vs target bottom-edge DoFs
    ax1 = Axis(fig[1, 1];
               title  = "PT iterations vs target bottom-edge DoFs ($nsteps steps)",
               xlabel = "target bottom-edge DoFs", ylabel = "PT iterations",
    ...
    # solver wall time vs actual number of DoFs
    ax2 = Axis(fig[1, 2];
