...
# Tuned on an RTX 3080. The ForwardDiff-heavy quadratic kernels use more
# registers per thread and reach better occupancy with smaller workgroups.
const cuda_workgroups = Dict(
    "Q1" => 256,
    "Q2" => 64,
    "T1" => 128,
    "T2" => 128,
...
const sizes  = [(32, 32), (128, 128), (256, 256), (512, 512), (1024, 1024), (1024, 1024).*2]
const nsteps = 10
...
function load_variant(name, file)
    ...
    Core.eval(mod, :(using CUDA))
    Core.eval(mod, :(SWEEP_BACKEND = CUDA.CUDABackend()))
    Core.eval(mod, :(SWEEP_WORKGROUP = $(cuda_workgroups[family])))
...
function summarize(results)
    ...
    Label(fig[0, 1:3], "Inclusion sweep — CUDA"; fontsize = 32)
    family_colors = Dict(
        family => color
    ...
    save(joinpath(@__DIR__, "inclusion_sweep_CUDA.png"), fig)
    display(fig)
