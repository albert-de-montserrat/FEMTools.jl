...
function compute_strain_rate_stress_postprocess(
    ...
    for iel in 1:nels
        ...
        for q in eachindex(geo_el)
            ...
            εII_q = √((εxx_dev^2 + εyy_dev^2 + εzz_dev^2) / 2 + εxy_q^2)
            τII_q = √((τxx_q^2 + τyy_q^2 + τzz_q^2) / 2 + τxy_q^2)
...
function main(; 
    ...
    τy      = 1 / cosd(30)                   # cohesion C; yield stress = C·cosϕ = 1.6
